      ...
      for source_path,destination_path in files_to_copy:
            try:
                 ...
                 if verify_hash(source_path,destination_path):
                      verified_count+=1
                      logging.info(f"Hash verified successfully: {file}")
                 else:
                      verification_failed_count+=1   
                      logging.error(f"Hash verification failed: {file} ")  
                 success_count+=1 
                 logging.info(f"File copied successfully: {file}") 
                 print(f"File copied successfully: {file}") 
                 logging.info(f"Backup completed of: {files}")
                 print(f"Backup completed of: {files}")
